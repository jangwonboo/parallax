...
2026-08-05 세션 — **페이지 검증에 Chandra 로컬 OCR 엔진 추가.** `pagecheck.py` 에 `--engine chandra`([datalab-to/chandra](https://github.com/datalab-to/chandra), 공개 OCR VLM) — Claude 비전의 「전 쪽 재판독」과 같은 일을 로컬 모델이 무료·키 없이 한다. 출력이 페이지 전체 markdown 이라 layer 신뢰 모드는 성립하지 않고 **항상 재구성**(`from_ocr|rebuilt`, 레이어는 `superseded` 보관)이다. markdown → rebuild 항목 변환은 `chandra_md_items()`(제목 h1·h2·h3, 인용, 파이프/HTML 표 → table_raw, 그림 버림). 백엔드는 `--chandra-method vllm`(기본, `chandra_vllm` 서버·`$VLLM_API_BASE`) | `hf`(로컬 PyTorch, `pip install chandra-ocr[hf]`). 앱은 검증 다이얼로그에 「전 쪽 재판독 (Chandra·로컬)」 버튼을 **파이썬에 chandra 가 있을 때만** 넣는다(`hasChandra()` — `find_spec` 만 본다, import 는 torch 를 끌어 무겁다). 키가 없으면 Claude 두 모드가 버튼에서 빠지고, 키 없이 Chandra 만 있어도 검증을 돌릴 수 있게 됐다(`ANTHROPIC_API_KEY` 는 vision 엔진일 때만 넘긴다). chandra 는 이 PC 에 미설치라 실행 검증은 미설치 오류 경로와 md 파서 단위 시험까지 — 실 판독 검증은 설치 후 `--pages 1-20` 으로 할 것. zip 재포장하면서 `{references,scripts,templates}` 가짜 디렉터리가 풀린 사본에 남아 있던 것도 지웠다.
