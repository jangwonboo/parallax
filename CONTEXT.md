...
2026-08-05 세션 — **페이지 검증에 Datalab API 엔진 추가.** 처음에 로컬 Chandra(`--engine chandra`, 공개 웨이트 + GPU)로 넣었다가 같은 날 사용자의 정정으로 **[Datalab API](https://www.datalab.to)**(Chandra 를 만든 곳의 호스팅 서비스, 공개 웨이트보다 정확한 개선판이 돌아간다 — README 벤치마크 86.7% vs 85.8%)로 교체했다. `pagecheck.py --engine datalab`: 쪽 PNG 를 `POST /api/v1/convert`(multipart, `X-API-Key`)로 보내고 `request_check_url` 을 2초 간격 폴링(5분 한도), markdown 을 받아 **항상 재구성**(`from_ocr|rebuilt`, 밀려난 레이어는 `superseded`) — layer 신뢰 모드는 출력이 페이지 전체 markdown 이라 성립하지 않는다. 변환은 `md_items()`(h1~h3·인용·파이프/HTML 표→table_raw·그림 버림), 모드는 `--datalab-mode fast|balanced|accurate`(기본 accurate), HTTP 는 anthropic 이 이미 끌어오는 httpx 라 **새 의존성 없음**. 키는 `DATALAB_API_KEY` — 환경변수 → `.env`(cwd → 스킬 폴더 → 그 부모) 순이고, 자리표시자(`your-datalab-api-key-here`)는 없는 것으로 친다. 루트에 `.env`(자리표시자, `.gitignore` 에 추가)와 `.env.example`(커밋됨)을 뒀다. 앱은 다이얼로그에 「전 쪽 재판독 (Datalab API)」 버튼을 **키가 읽힐 때만** 넣고(`readDatalabKey()` — 앱이 직접 읽어 자식 환경변수로 넘긴다, 패키징되면 cwd 가 달라서), Anthropic 키가 없어도 Datalab 키만 있으면 검증이 돈다. **실 API 호출은 미검증**(자리표시자 키뿐) — 키를 넣고 `--pages 1-20` 으로 시험할 것. zip 재포장하면서 `{references,scripts,templates}` 가짜 디렉터리가 풀린 사본에 남아 있던 것도 지웠다.
