...
## 2026-07-28 오후 — v2 유실과 재건

**`the_meaning_of_your_life_v2.parallax` 가 13:54 에 0바이트로 잘렸다. 원인 미상.** 직전까지 파이썬(sqlite3)으로 블록을 고치고 있었고 앱 인스턴스도(구 빌드 포함 여러 개) 떠 있었다 — 어느 쪽인지 특정하지 못했다. SQLite 파일 하나를 여러 프로세스가 물고 있는 동안 밖에서 고치는 일은 피할 것.

**13:50 에 마침 새 내보내기(영·한 md 두 파일)를 돌린 덕에 전체 스냅샷이 있었다.** 두 파일은 블록 순서를 그대로 공유하므로 1:1 로 짝지어 재건했다(955블록, 유형은 md 마커를 역해석 — en/ko 불일치 0). 잃은 것: 쪽 번호(전부 NULL), 세부 플래그(`from_ocr` 만 전 블록에 복원), 용어집·superseded. **md 내보내기가 사실상 유일한 백업 수단이다 — 번역이 진행되면 가끔 내보내 둘 것.**

재건 후 손본 것: 챕터 3 「Questions for Reflection and Self-Assessment」 구간이 원래부터 깨져 있던 것을 PDF 67쪽과 대조해 수리했다(66→67쪽 경계에서 단어 중간 `diffi|cult` 로 잘린 단락 복원, 문항에 파묻힌 제목을 별도 h2 로, 쪽 경계·단 경계로 쪼개진 문항 8개 병합, `¬` 분철·OCR 쓰레기 `'tri` 정리, 문항 유형 footnote→p 교정). 제목 번역을 통째로 감싼 `**…**` 마크업도 걷어냈다. 최종 948블록.

---

## 남아 있는 데이터 파일

작업 파일은 **`the_meaning_of_your_life_v2.parallax` 하나**다(**md 스냅샷에서 재건한 948블록, 번역 전량**). 블록 ID 는 재건 때 새로 매겼으므로 예전 세션의 ID 와 대응하지 않는다. 재건 원본인 `the_meaning_of_your_life_compressed.en.md`/`.ko.md`(2026-07-28 13:50)는 백업으로 남겨 둔다 — 지우지 말 것.

재건 전에 손봤던 것(스냅샷에 반영되어 살아 있다): 챕터 7 자기평가 문항 5~8 이 h2 로 잘못 분류되어 목차에 오르던 것을 PDF 151쪽과 대조해 `p` 로 교정, 4번 문항 뒷부분 중복 잔해 블록 삭제, 번역 앞에 흘린 `h2 ` 토큰 제거.
