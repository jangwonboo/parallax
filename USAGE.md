...
`extract.py` 와 `pagecheck.py` 는 끝에서 제목 레벨을 다시 고른다. 페이지마다 레벨이 달라져 목차에서 장이 절보다 아래에 놓이는 것을 막기 위한 것인데, 기준은 **책이 앞머리에 스스로 적어 둔 Contents** 다 — 어느 제목이 장이고 어느 것이 그 아래인지는 거기에만 적혀 있다. 인쇄 목차가 부(部)와 장(章)에 따로 번호를 매기면 장을 부 아래로 넣어 세 단, 절까지 있으면 네 단이 된다. 깊이는 책마다 다르다.

같은 정답지로 목차를 더럽히는 것들도 함께 걷어낸다 — 표지 글자, 도판 안에 인쇄된 제목, 도판 번호표(`EXHIBIT 2.1`), 뒤쪽 주석·색인 아래 세부 항목, 그리고 **인쇄된 목차 쪽 자체**(목차는 앱 왼쪽 패널이 전담한다). 수사가 붙은 제목은 `수사 · 제목` 한 꼴로 맞춘다.

원본 레벨이 필요하면 두 스크립트 모두 `--keep-heading-levels` 를 받는다. 자세한 규칙은 스킬의 `SKILL.md` ①.
...
| 제목 레벨에 건너뛴 단이 있음(h1 다음 h3) | 정규화가 안 돌았다 — `--keep-heading-levels` 를 준 게 아닌지 확인 |
| 블록이 하나도 없는 쪽이 있음 | 판독이 그 쪽을 놓쳤다. pagecheck 가 끝에서 찍어 주는 명령으로 그 쪽만 다시 읽어라. **원본을 눈으로 먼저 볼 것** — 진짜 색인·공백 쪽도 걸린다 |
| 목차 맨 위가 책 제목의 되풀이 | 표지 강등이 안 걸렸다. 인쇄 목차를 못 찾은 책이다(`toc_pages` 가 빈 집합) |
