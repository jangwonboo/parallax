...
`pdf-ko-translate` 파이프라인과 병렬 대역 리더를 Electron 앱의 엔진·뷰로 편입하기 위한 설계 문서.

> **이 문서의 위치.** 기존 Parallax 스펙 M3의 엔진·데이터 계층을 확정하고, UI·사용자 흐름은 `pdf-ko-translate`가 내는 병렬 대역 리더를 그대로 채택한다. M3의 톤앤매너 템플릿 UI는 폐기한다(§0.1).
...
이미 판독한 페이지를 다시 돌리지 않기 위한 기록이다. 비어 있으면 pagecheck를 한 번도 돌리지 않은 문서다.
